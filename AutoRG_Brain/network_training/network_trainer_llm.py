...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.patience = 50
        self.val_eval_criterion_alpha = 0.1  # alpha * old + (1-alpha) * new
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        start_time = time()
        state_dict = self.llm_model.state_dict()
        for key in state_dict.keys():
            state_dict[key] = state_dict[key].cpu()
        # seg_state_dict = None
        # if self.train_with_seg or self.use_conv_pool or self.use_patchwise:
        seg_state_dict = self.network.state_dict()
        for key in seg_state_dict.keys():
            seg_state_dict[key] = seg_state_dict[key].cpu()
        ...
        save_this = {
            ...
            'seg_model': seg_state_dict,
            'optimizer_state_dict': optimizer_state_dict,
            'lr_scheduler_state_dict': lr_sched_state_dct,
            'plot_stuff': (self.all_tr_losses, self.all_val_losses, self.all_val_losses_tr_mode,
                           self.all_val_eval_metrics, self.all_train_eval_metrics),
    ...
    def load_latest_checkpoint(self, train=True):
        if isfile(join(self.output_folder, "model_final_checkpoint.model")):
            print("load",join(self.output_folder, "model_final_checkpoint.model"))
        ...
        if isfile(join(self.output_folder, "model_best_rouge.model")):
            print("load",join(self.output_folder, "model_best_rouge.model"))
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        try:
            self.llm_model.load_state_dict(checkpoint['state_dict'])
        except:
            new_state_dict = OrderedDict()
            curr_state_dict_keys = list(self.llm_model.state_dict().keys())
            for k, value in checkpoint['state_dict'].items():
                key = k
                if key not in curr_state_dict_keys and 'module.'+key in curr_state_dict_keys:
                    # print(ao)
                    # key = key[7:]
                    key = 'module.'+key
                new_state_dict[key] = value

            self.llm_model.load_state_dict(new_state_dict)
        ...
        self.all_tr_losses, self.all_val_losses, self.all_val_losses_tr_mode,self.all_val_eval_metrics, self.all_train_eval_metrics = checkpoint[
            'plot_stuff']
        ...
        if train and self.epoch != len(self.all_tr_losses):
            ...
            for v in self.all_val_eval_metrics:
                self.all_val_eval_metrics[v] = self.all_val_eval_metrics[v][:self.epoch]
    ...
    def update_eval_criterion_MA(self):
        ...
        for v in self.val_eval_criterion_MA:

            if self.val_eval_criterion_MA[v] is None:
                if len(self.all_val_eval_metrics[v]) == 0:
                    self.val_eval_criterion_MA[v] = - self.all_val_losses[-1]
                else:
                    self.val_eval_criterion_MA[v] = self.all_val_eval_metrics[v][-1]
            else:
                if len(self.all_val_eval_metrics[v]) == 0:
                    """
                    ...
                    """
                    self.val_eval_criterion_MA[v] = self.val_eval_criterion_alpha * self.val_eval_criterion_MA[v] - (
                            1 - self.val_eval_criterion_alpha) * \
                                                self.all_val_losses[-1]
                else:
                    self.val_eval_criterion_MA[v] = self.val_eval_criterion_alpha * self.val_eval_criterion_MA[v] + (
                            1 - self.val_eval_criterion_alpha) * \
                                                self.all_val_eval_metrics[v][-1]
        ...
        if self.patience is not None:
            ...
            for v in self.best_val_eval_criterion_MA:
                ...
                if self.val_eval_criterion_MA[v] > self.best_val_eval_criterion_MA[v]:
                    ...
                    if self.save_best_checkpoint and v=='rouge': self.save_checkpoint(join(self.output_folder, "model_best_"+v+".model"))
