...
class NetworkTrainer(object):
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        """
        Save a checkpoint. This will also save the optimizer and scheduler state if requested.
        """
        save_dict = {
            'epoch': self.epoch,
            'all_tr_losses': self.all_tr_losses,
            'all_val_losses': self.all_val_losses,
            'all_val_eval_metrics': getattr(self, 'all_val_eval_metrics', []),
            'all_val_eval_metrics_ana': getattr(self, 'all_val_eval_metrics_ana', {}),
            'all_val_eval_metrics_ab': getattr(self, 'all_val_eval_metrics_ab', {}),
            'all_train_eval_metrics_ana': getattr(self, 'all_train_eval_metrics_ana', []),
            'all_train_eval_metrics_ab': getattr(self, 'all_train_eval_metrics_ab', []),
            'val_eval_criterion_MA': getattr(self, 'val_eval_criterion_MA', {}),
            'val_eval_criterion_MA_ana': getattr(self, 'val_eval_criterion_MA_ana', {}),
            'val_eval_criterion_MA_ab': getattr(self, 'val_eval_criterion_MA_ab', {}),
            'best_val_eval_criterion_MA': getattr(self, 'best_val_eval_criterion_MA', {}),
            'best_val_eval_criterion_MA_ana': getattr(self, 'best_val_eval_criterion_MA_ana', {}),
            'best_val_eval_criterion_MA_ab': getattr(self, 'best_val_eval_criterion_MA_ab', {}),
        ...
        torch.save(save_dict, fname + ".model")
        write_pickle(save_dict, fname + ".model.pkl")
    ...
    def load_latest_checkpoint(self, train=True):
        ...
        if isfile(join(self.output_folder, "model_latest.model")):
            print("load",join(self.output_folder, "model_latest.model"))
    ...
    def load_checkpoint(self, fname):
        """
        Load a checkpoint. This will also load the optimizer and scheduler state if present.
        """
        if not isfile(fname + ".model"):
            raise RuntimeError("No checkpoint found at %s" % fname)

        self.print_to_log_file("loading checkpoint", fname)

        checkpoint = torch.load(fname + ".model", map_location=torch.device('cpu'))
        ...
        # === SAFE METRIC LOADING ===
        self.all_val_eval_metrics = checkpoint.get('all_val_eval_metrics', [])
        self.all_val_eval_metrics_ana = checkpoint.get('all_val_eval_metrics_ana', {})
        self.all_val_eval_metrics_ab = checkpoint.get('all_val_eval_metrics_ab', {})
        self.all_train_eval_metrics_ana = checkpoint.get('all_train_eval_metrics_ana', [])
        self.all_train_eval_metrics_ab = checkpoint.get('all_train_eval_metrics_ab', [])
        self.val_eval_criterion_MA = checkpoint.get('val_eval_criterion_MA', {})
        self.val_eval_criterion_MA_ana = checkpoint.get('val_eval_criterion_MA_ana', {})
        self.val_eval_criterion_MA_ab = checkpoint.get('val_eval_criterion_MA_ab', {})
        self.best_val_eval_criterion_MA = checkpoint.get('best_val_eval_criterion_MA', {})
        self.best_val_eval_criterion_MA_ana = checkpoint.get('best_val_eval_criterion_MA_ana', {})
        self.best_val_eval_criterion_MA_ab = checkpoint.get('best_val_eval_criterion_MA_ab', {})
        ...
        self.print_to_log_file("checkpoint loaded")
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        if train:
            self.all_tr_losses, self.all_val_losses, self.all_val_losses_tr_mode,self.all_val_eval_metrics_ana, self.all_val_eval_metrics_ab, self.all_train_eval_metrics_ana, self.all_train_eval_metrics_ab, self.val_eval_criterion_MA = checkpoint[
                'plot_stuff']

            # load best loss (if present)
            if 'best_stuff' in checkpoint.keys():
                self.best_epoch_based_on_MA_tr_loss, self.best_MA_tr_loss_for_patience, self.best_val_eval_criterion_MA_ana, self.best_val_eval_criterion_MA_ab, self.best_val_eval_criterion_MA = checkpoint[
                    'best_stuff']
        ...
        if self.epoch != len(self.all_tr_losses):
            ...
            try:
                for v in self.all_val_eval_metrics_ana:
                    self.all_val_eval_metrics_ana[v] = self.all_val_eval_metrics_ana[v][:self.epoch]
                for v in self.all_val_eval_metrics_ab:
                    self.all_val_eval_metrics_ab[v] = self.all_val_eval_metrics_ab[v][:self.epoch]
            except:
                pass
    ...
    def maybe_save_checkpoint(self):
        ...
        if self.save_intermediate_checkpoints and (self.epoch % self.save_every == (self.save_every - 1)):
            ...
            self.print_to_log_file("done saving model_ep_%03.0d.model" % (self.epoch + 1))
    ...
    def update_eval_criterion_MA(self):
        ...
        for v in self.val_eval_criterion_MA_ab:

            if self.val_eval_criterion_MA_ab[v] is None:
                if len(self.all_val_eval_metrics_ab[v]) == 0:
                    self.val_eval_criterion_MA_ab[v] = - self.all_val_losses[-1]
                else:
                    self.val_eval_criterion_MA_ab[v] = self.all_val_eval_metrics_ab[v][-1]
            else:
                if len(self.all_val_eval_metrics_ab[v]) == 0:
                    """
                    ...
                    """
                    self.val_eval_criterion_MA_ab[v] = self.val_eval_criterion_alpha * self.val_eval_criterion_MA_ab[v] - (
                            1 - self.val_eval_criterion_alpha) * \
                                                self.all_val_losses[-1]
                else:
                    self.val_eval_criterion_MA_ab[v] = self.val_eval_criterion_alpha * self.val_eval_criterion_MA_ab[v] + (
                            1 - self.val_eval_criterion_alpha) * \
                                                self.all_val_eval_metrics_ab[v][-1]
        
        for v in self.val_eval_criterion_MA_ana:

            if self.val_eval_criterion_MA_ana[v] is None:
                if len(self.all_val_eval_metrics_ana[v]) == 0:
                    self.val_eval_criterion_MA_ana[v] = - self.all_val_losses[-1]
                else:
                    self.val_eval_criterion_MA_ana[v] = self.all_val_eval_metrics_ana[v][-1]
            else:
                if len(self.all_val_eval_metrics_ana[v]) == 0:
                    """
                    We here use alpha * old - (1 - alpha) * new because new in this case is the vlaidation loss and lower
    ...
    def update_eval_criterion_MA_six_pub(self):
        """
        If self.all_val_eval_metrics is unused (len=0) then we fall back to using -self.all_val_losses for the MA to determine early stopping
        (not a minimization, but a maximization of a metric and therefore the - in the latter case)
        :return:
    ...
    def manage_patience_six_pub(self):
        ...
        if self.patience is not None:
            ...
            for v in self.best_val_eval_criterion_MA_ab:
                if self.best_val_eval_criterion_MA_ab[v] is None:
                    self.best_val_eval_criterion_MA_ab[v] = self.val_eval_criterion_MA_ab[v]
                
                if self.val_eval_criterion_MA_ab[v] > self.best_val_eval_criterion_MA_ab[v]:
                    self.best_val_eval_criterion_MA_ab[v] = self.val_eval_criterion_MA_ab[v]
                    #self.print_to_log_file("saving best epoch checkpoint...")
                    # if self.save_best_checkpoint: self.save_checkpoint(join(self.output_folder, "model_best_ab_"+v+".model"))
            
            for v in self.best_val_eval_criterion_MA_ana:
                if self.best_val_eval_criterion_MA_ana[v] is None:
                    self.best_val_eval_criterion_MA_ana[v] = self.val_eval_criterion_MA_ana[v]
                
                if self.val_eval_criterion_MA_ana[v] > self.best_val_eval_criterion_MA_ana[v]:
                    self.best_val_eval_criterion_MA_ana[v] = self.val_eval_criterion_MA_ana[v]
                    #self.print_to_log_file("saving best epoch checkpoint...")
                    # if self.save_best_checkpoint: self.save_checkpoint(join(self.output_folder, "model_best_ana_"+v+".model"))

            for v in self.best_val_eval_criterion_MA:
                if self.best_val_eval_criterion_MA[v] is None:
