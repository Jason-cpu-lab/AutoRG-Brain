...
class nnUNetTrainerV2(nnUNetTrainer):
    ...
    def __init__(self, plans_file, fold, train_file, only_ana=False, abnormal_type="intense", num_batches_per_epoch=250, num_val_batches_per_epoch=50, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        # === Initialize online evaluation lists ===
        self.online_eval_foreground_dc_ana = []
        self.online_eval_tp_ana = []
        self.online_eval_fp_ana = []
        self.online_eval_fn_ana = []
        ...
        # === SAFE INITIALIZATION OF METRICS ===
        ...
        if self.test_file is not None:
            ...
            self.val_eval_criterion_MA_ana = {i: None for i in self.test_file}
            self.val_eval_criterion_MA_ab = {i: None for i in self.test_file}
            self.val_eval_criterion_MA = {'ab': None, 'ana': None, 'both': None}
            self.best_val_eval_criterion_MA_ana = {i: None for i in self.test_file}
            self.best_val_eval_criterion_MA_ab = {i: None for i in self.test_file}
            self.best_val_eval_criterion_MA = {'ab': None, 'ana': None, 'both': None}
        else:
            # Fallback when no test_file
            self.all_val_eval_metrics_ana = {}
            self.all_val_eval_metrics_ab = {}
            self.val_eval_criterion_MA_ana = {}
            self.val_eval_criterion_MA_ab = {}
            self.val_eval_criterion_MA = {'ab': 0.0, 'ana': 0.0, 'both': 0.0}
            self.best_val_eval_criterion_MA_ana = {}
            self.best_val_eval_criterion_MA_ab = {}
            self.best_val_eval_criterion_MA = {'ab': 0.0, 'ana': 0.0, 'both': 0.0}
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if not self.was_initialized:
            ...
            if training:
                ...
                #print("self.dl_tr",self.dl_tr)
                #print("self.dl_val",self.dl_val)
                #print("self.dl_tr",self.dl_tr.list_of_keys_modal)
    ...
    def finish_online_evaluation(self, mode="train"):
        ...
        global_dc_per_class_ana = [i for i in [2 * i / (2 * i + j + k) for i, j, k in
                               ...
                               if not np.isnan(i)]
        if mode == "train":
            self.all_train_eval_metrics_ana.append(np.nanmean(global_dc_per_class_ana))
        else:
            if self.test_file is not None:
                self.all_val_eval_metrics_ana['test'].append(np.nanmean(global_dc_per_class_ana))
        ...
        global_dc_per_class_ab = [i for i in [2 * i / (2 * i + j + k) for i, j, k in
                               ...
                               if not np.isnan(i)]
        if mode == "train":
            self.all_train_eval_metrics_ab.append(np.nanmean(global_dc_per_class_ab))
        else:
            if self.test_file is not None:
                self.all_val_eval_metrics_ab['test'].append(np.nanmean(global_dc_per_class_ab))
