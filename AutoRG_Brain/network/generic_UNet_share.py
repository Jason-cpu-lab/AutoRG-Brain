...
class Generic_UNet(SegmentationNetwork):
    ...
    def forward(self, x, modal):
        ...
        for u in range(len(self.tu)):
            ...
            seg_outputs_anatomy.append(self.final_nonlin(self.seg_outputs_anatomy[u](x)))
            seg_outputs_abnormal.append(self.final_nonlin(self.seg_outputs_abnormal[u](x)))

        if self._deep_supervision and self.do_ds:
            return tuple([seg_outputs_anatomy[-1]] + [i(j) for i, j in
                                              zip(list(self.upscale_logits_ops)[::-1], seg_outputs_anatomy[:-1][::-1])]), tuple([seg_outputs_abnormal[-1]] + [i(j) for i, j in
                                              zip(list(self.upscale_logits_ops)[::-1], seg_outputs_abnormal[:-1][::-1])])
        else:
