...
def get_case_identifiers(folder):
    print('dataset_loading_llm.py: get_case_identifiers')
...
def unpack_dataset(folder, threads=default_num_threads, key="data",train_file = None):
    """
...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            ### reverse anatomy label ###
            # seg_reverse = np.zeros(case_all_data[-1:].shape)
            # for from_label, to_label in self.reverse_anatomy_map.items():
            #     seg_reverse[case_all_data[-1:] == int(from_label)] = int(to_label)
            # seg[j, 0] = np.pad(seg_reverse, ((0, 0),
            #                                 (-min(0, bbox_x_lb), max(bbox_x_ub - shape[0], 0)),
            #                                 (-min(0, bbox_y_lb), max(bbox_y_ub - shape[1], 0)),
            #                                 (-min(0, bbox_z_lb), max(bbox_z_ub - shape[2], 0))),
        ...
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys, 'modal':modal, "report":reports}
