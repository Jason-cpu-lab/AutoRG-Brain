...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, abnormal_type="intense", has_prev_stage=False,
        ...
        # === DEBUG: Check all unique suffixes ===
        print("\n--- DEBUG: Unique suffixes in keys ---")
        suffixes = set()
        ...
        'a': list(filter(lambda x: x.endswith('_dwi_ana') or x.endswith('_ses-0001_dwi_ana'), self.list_of_keys)),
        'b': list(filter(lambda x: x.endswith('_t1_ana') or x.endswith('-t1n_ana'), self.list_of_keys)),
        'c': list(filter(lambda x: x.endswith('_t2_ana') or x.endswith('-t2w_ana'), self.list_of_keys)),
        'd': list(filter(lambda x: x.endswith('_flair_ana') or x.endswith('-t2f_ana'), self.list_of_keys))
        }
        
        print("\n--- DEBUG: Filter Results ---")
        for modal, filter_func in [('a', lambda x: x.endswith('_dwi_ana') or x.endswith('_ses-0001_dwi_ana')),
                               ('b', lambda x: x.endswith('_t1_ana') or x.endswith('-t1n_ana')),
                               ('c', lambda x: x.endswith('_t2_ana') or x.endswith('-t2w_ana')),
                               ('d', lambda x: x.endswith('_flair_ana') or x.endswith('-t2f_ana'))]:
            matched = list(filter(filter_func, self.list_of_keys))
            print(f"Modal {modal} matched {len(matched)} cases")
            if len(matched) > 0:
                print(f"  Examples: {matched[:3]}")
        print("--- END DEBUG ---\n")


        print("\n--- DEBUG: DataLoader3D Modality Check ---")
        print(f"Total cases loaded: {len(self.list_of_keys)}")
        print(f"Modality 'a' (DWI) case count: {len(self.list_of_keys_modal['a'])}")
        print(f"Modality 'b' (T1WI) case count: {len(self.list_of_keys_modal['b'])}")
        print(f"Modality 'c' (T2WI) case count: {len(self.list_of_keys_modal['c'])}")
        print(f"Modality 'd' (FLAIR) case count: {len(self.list_of_keys_modal['d'])}")
        print("------------------------------------------\n")

        # === DEBUG: Check for unmatched cases ===
        matched_keys = set()
        for modal_keys in self.list_of_keys_modal.values():
            matched_keys.update(modal_keys)
        unmatched_keys = set(self.list_of_keys) - matched_keys
        print(f"Unmatched cases: {len(unmatched_keys)}")
        if unmatched_keys:
            print(f"Unmatched examples: {list(unmatched_keys)[:5]}")
        print("------------------------------------------\n")
    ...
    def determine_shapes(self):
        ...
        # CHANGE THIS LINE:
        # num_color_channels = case_all_data.shape[0] - 1  # OLD - assumes n-1 channels
        num_color_channels = 1  # NEW - we're only using 1 channel now
    ...
    def generate_train_batch(self):
        ...
        if self.list_of_keys[0][3]!='_':
            ...
            # --- FIX: Dynamically select a non-empty modality list to prevent errors ---
            available_modals = [m for m, keys in self.list_of_keys_modal.items() if len(keys) > 0]
            for modal in available_modals:

             if not available_modals:
                raise ValueError("No cases loaded for any modality ('a', 'b', 'c', 'd'). Check your data loading keys and filters.")
            
            # Choose one of the available modalities with equal probability
            choose_modal = np.random.choice(available_modals)
            selected_keys = np.random.choice(self.list_of_keys_modal[choose_modal], self.batch_size, True, None)
        ...
        for j, i in enumerate(selected_keys):
            ...
            name = self._data[i]['data_file'][:-4].split('/')[-1].split('_')[0]

            anatomy_scan = case_all_data[1]
            anatomy_scan[anatomy_scan<0] = 0

            flag = 0

            if self.abnormal_type != "no_abnormal":
                ...
                while(flag == 0 and cnt<15):
                    try:
                        if self.abnormal_type == "intense" or (self.abnormal_type == "mix" and np.random.rand()<0.5):
                            ...
                            # Note: SynthesisTumor_copypaste is commented out in imports, ensure it's available if needed.
            ...
            seg_from_previous_stage = np.expand_dims(seg_from_previous_stage, axis=0)
            ...
            if not force_fg:
                bbox_x_lb = np.random.randint(lb_x, ub_x + 1) # 下界取值范围
            ...
            data[j] = np.pad(case_all_data[0:1], ((0, 0),  # Take only channel 0 (image)
        ...
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys, 'modal':modal}


class MemoryOptimizedTrainer:
    """Memory optimized training wrapper"""
    ...
    def train_epoch(self, dataloader, epoch):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            seg = torch.from_numpy(batch['seg']).to(self.device, non_blocking=True)
            ...
            if batch_idx % 10 == 0:
                self.print_memory_usage(epoch, batch_idx)
                
        return total_loss / len(dataloader)
    
    def print_memory_usage(self, epoch, batch_idx):
...
def create_memory_optimized_dataloader(dataset, batch_size=2, patch_size=(128, 128, 128)):
    """
    Create a memory-optimized data loader with reduced patch size and batch size
    """
    # Clear cache before creating data loader
    torch.cuda.empty_cache()
    
    dataloader = DataLoader3D(
        dataset=dataset,
        patch_size=patch_size,  # Reduced from (224, 228, 189)
        final_patch_size=patch_size,
        batch_size=batch_size,  # Reduced from 8
...
def main_training_loop():
    ...
    # Load dataset
    t = "Task002_Heart"  # Replace with your task name
    p = join(preprocessing_output_dir, t, "stage1")
    dataset = load_dataset(p)
    
    # Unpack dataset for faster loading
    unpack_dataset(p)
    
    # Create memory-optimized data loader
    print("Creating memory-optimized data loader...")
    dataloader = create_memory_optimized_dataloader(
        dataset, 
        batch_size=2,  # Reduced from 8
        patch_size=(128, 128, 128)  # Reduced from ~224
    )
    
    # Initialize model, optimizer, loss function (replace with your actual implementations)
    # model = YourModel().cuda()
    # optimizer = torch.optim.Adam(model.parameters())
    # loss_fn = YourLossFunction()
    
    # Create trainer
    # trainer = MemoryOptimizedTrainer(model, optimizer, loss_fn, device='cuda')
    
    print("Memory optimized training setup complete!")
    print(f"Batch size: 2 (reduced from 8)")
    print(f"Patch size: (128, 128, 128) (reduced from ~224)")
    print(f"Expected memory reduction: ~95%")


if __name__ == "__main__":
    # Run the main training setup
    main_training_loop()
    
    # Original test code (optional)
    t = "Task002_Heart"
    p = join(preprocessing_output_dir, t, "stage1")
    dataset = load_dataset(p)
    ...
    dl = DataLoader3D(dataset, (64, 64, 64), (64, 64, 64), 2, oversample_foreground_percent=0.33)
    
    # For 2D (if needed)
    from batchgenerators.dataloading.data_loader import DataLoader2D
    dl2d = DataLoader2D(dataset, (64, 64), (64, 64), 4, oversample_foreground_percent=0.33)